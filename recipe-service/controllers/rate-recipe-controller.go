 ...
 */
type RateRecipeController struct {
}

/*
 * This is a simple implementation of the type Controller which will return the request handler function
 */
func (cont *RateRecipeController) Apply(service core.RecipeService) core.RequestContext {
	return func(writer http.ResponseWriter, request *http.Request) {
		...
		id := (core.ID)(vars["id"])
		rating, err := strconv.ParseInt(vars["rating"], 10, 64)
		if err != nil || rating > 5 || rating < 1 {
			util.BadRequest(model.ErrorInvalidRequest)(writer,request)
			return
		}
		err = service.RateRecipe(id, int(rating))
		if err != nil {
